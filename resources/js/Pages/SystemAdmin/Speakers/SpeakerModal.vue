    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ speaker ? t('admin.speakers.edit') : t('admin.speakers.create') }}
                    </h3>
            ...
            <form @submit.prevent="submit" class="p-6 space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        {{ t('admin.form.avatar_url') }}
                    </label>
                    <input v-model="form.avatar_url"
                           type="url"
                           :placeholder="t('admin.form.placeholder.avatar_url')"
                           class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-opacity-50 transition-colors"
                           :style="{ '--tw-ring-color': themeColor.primary }">
                </div>
                ...
                <div class="flex justify-end gap-4 pt-4">
                    ...
                    <button type="submit"
                        ...
                        {{ form.processing ? t('admin.actions.saving') : (speaker ? t('admin.actions.update') : t('admin.speakers.create')) }}
                    </button>
                </div>
            </form>
        </div>
...
const form = useForm({
    ...
    avatar_url: props.speaker?.avatar_url || '',
...
watch(() => props.speaker, (newSpeaker) => {
    if (newSpeaker) {
        form.name = newSpeaker.name || ''
        form.email = newSpeaker.email || ''
        form.phone = newSpeaker.phone || ''
        form.title = newSpeaker.title || ''
        form.organization_id = newSpeaker.organization_id || ''
        form.bio = newSpeaker.bio || ''
        form.avatar_url = newSpeaker.avatar_url || ''
        form.linkedin = newSpeaker.linkedin || ''
        form.twitter = newSpeaker.twitter || ''
        form.website = newSpeaker.website || ''
        form.expertise = newSpeaker.expertise || ''
    }
})
