...
const form = useForm({
    ...
    theme: props.edition.theme,
    registration_start_date: props.edition.registration_start_date,
    registration_end_date: props.edition.registration_end_date,
    idea_submission_start_date: props.edition.idea_submission_start_date,
    idea_submission_end_date: props.edition.idea_submission_end_date,
    event_start_date: props.edition.event_start_date,
    event_end_date: props.edition.event_end_date,
    location: props.edition.location,
    status: props.edition.status,
    is_current: props.edition.is_current,
    created_by: props.edition.created_by
})

const associatedWorkshops = ref(props.edition.workshops || [])
                ...
                <div class="self-stretch flex flex-row items-start justify-between flex-wrap content-start p-4">
                    <div class="flex flex-col items-start justify-start gap-3 min-w-[288px]">
                        <div class="w-[421px] flex flex-col items-start justify-start">
                            <h1 class="text-[32px] leading-10 font-bold text-gray-900 dark:text-white font-space-grotesk">Edit Edition Details</h1>
                        </div>
                ...
                <form @submit.prevent="submitForm" class="w-full">
                        ...
                        <div class="w-[480px] flex flex-row items-end justify-start flex-wrap content-end py-3 px-4 box-border gap-4 max-w-[480px]">
                            <div class="flex-1 flex flex-col items-start justify-start min-w-[160px]">
                                ...
                                <select v-model="form.created_by"
                                    ...
                                    <option :value="edition.created_by">{{ edition.creator?.name || 'Select Admin' }}</option>
                    ...
                    <div class="self-stretch flex flex-row items-start justify-end py-3 px-4 gap-3 text-center text-sm">
                        ...
                        <button type="submit"
                            ...
                            <div class="overflow-hidden flex flex-col items-center justify-start">
                                <span class="self-stretch relative leading-[21px] font-bold overflow-hidden text-ellipsis whitespace-nowrap text-white">{{ form.processing ? 'Saving...' : 'Update Edition' }}</span>
                            </div>
                        </button>
                    </div>
                </form>
