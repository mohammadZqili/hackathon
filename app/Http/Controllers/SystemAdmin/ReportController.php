...
<body>
    ...
    {
        $editionId = $request->get('edition_id');
        $stats = $this->reportService->getEditionReport($editionId ?? 1);

        return Inertia::render('SystemAdmin/Reports/Teams', [
            'stats' => $stats['team_performance'] ?? []
        ]);
    }
    ...
    {
        $editionId = $request->get('edition_id', 1);
        $report = $this->reportService->getEditionReport($editionId);

        return Inertia::render('SystemAdmin/Reports/Ideas', [
            'stats' => $report['idea_status'] ?? []
        ]);
    }
    ...
    {
        $editionId = $request->get('edition_id');
        $metrics = $this->reportService->getWorkshopMetrics($editionId);
    ...
    {
        $request->validate([
            'type' => 'required|in:overall,edition,comparison',
            'edition_ids' => 'required_if:type,edition,comparison|array',
            'format' => 'required|in:pdf,excel,csv'
        ]);

        $type = $request->get('type');
        $format = $request->get('format');
        $editionIds = $request->get('edition_ids', []);

        $data = match($type) {
            'overall' => $this->reportService->generateExportData(),
            'edition' => $this->reportService->getEditionReport($editionIds[0] ?? 1),
            'comparison' => $this->reportService->compareEditions($editionIds),
            default => []
        };
    ...
    /**
     * Schedule reports for regular generation
     */
    public function scheduleReports(Request $request)
    {
        $request->validate([
            ...
            'recipients.*' => 'email',
            'report_type' => 'required|in:overall,edition',
            'edition_id' => 'required_if:report_type,edition'
        ]);

        // In production, this would schedule reports using Laravel's task scheduling
        return response()->json([
            'success' => true,
